...
use crate::schema::{
    region_ownership, territory_adjacency, territory_ownership, turn, user,
};
    ...
    conn.run(move |connection| {
        UpdateUser::do_update(
            UpdateUser {
                ...
                turn: user_stats.totalTurns,
                game_turn: user_stats.gameTurns,
...
// Return the # of regions owned by `team` on `latest` turn.
...
pub(crate) fn insert_turn(
    ...
    diesel::insert_into(turn::table)
        .values((
            turn::user_id.eq(user.1),
            turn::turn_id.eq(latest.id),
            turn::territory.eq(target),
            turn::mvp.eq(false),
            turn::power.eq(power),
            turn::multiplier.eq(multiplier),
            turn::weight.eq(user_weight),
            turn::stars.eq(user_ratings.overall),
            turn::team.eq(user.0),
            turn::alt_score.eq(alt_score),
            turn::merc.eq(merc),
        ))
        .on_conflict((turn::user_id, turn::turn_id))
        .do_update()
        .set((
            turn::alt_score.eq(alt_score),
            turn::territory.eq(target),
...
pub(crate) fn update_user(
    ...
    match new {
        false => diesel::update(user::table)
            .filter(user::id.eq(user))
            .set(user::current_team.eq(team))
            .execute(conn),
        true => diesel::update(user::table)
            .filter(user::id.eq(user))
            .set(user::playing_for.eq(team))
            .execute(conn),
    }
}
#[cfg(test)]

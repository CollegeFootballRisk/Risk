...
(function(m, q) { var u = "DataTable"; "object" == typeof exports ? module.exports = q(u) : "function" == typeof define && define.amd ? define([], q(u)) : m[u] = q(u) })("undefined" == typeof global ? this.window || this.global : global, function() {
    "use strict";
    var q = window,
        u = document,
        z = u.body,
        ...
        I = function(T, U) {
            if (T instanceof NodeList) F(T, function(V) { I(V, U) });
            else if (U)
    ...
    P.prototype.swap = function(T) {
        if (T.length && 2 === T.length) {
            var U = [];
            F(this.dt.headings, function(Y, Z) { U.push(Z) });
            var V = T[0],
                W = T[1],
                X = U[W];
        ...
        var ea = [];
        F(Y, function(fa, ga) { W.data.push(fa.row), null !== fa.row.searchIndex && void 0 !== fa.row.searchIndex && ea.push(ga) }, W), W.searchData = ea, this.rebuild(), W.update(), V || W.emit("datatable.sort", T, X)
    }, P.prototype.rebuild = function() {
        var T, U, V, W, X = this.dt,
            Y = [];
        X.activeRows = [], X.activeHeadings = [], F(X.headings, function(Z, $) { Z.originalCellIndex = $, Z.sortable = "false" !== Z.getAttribute("data-sortable"), 0 > X.hiddenColumns.indexOf($) && X.activeHeadings.push(Z) }, this), F(X.data, function(Z, $) { T = Z.cloneNode(), U = Z.cloneNode(), T.dataIndex = U.dataIndex = $, null !== Z.searchIndex && void 0 !== Z.searchIndex && (T.searchIndex = U.searchIndex = Z.searchIndex), F(Z.cells, function(_) { V = _.cloneNode(!0), V.data = _.data, T.appendChild(V), 0 > X.hiddenColumns.indexOf(_.cellIndex) && (W = _.cloneNode(!0), W.data = _.data, U.appendChild(W)) }), Y.push(T), X.activeRows.push(U) }), X.data = Y, X.update()
    };
    var Q = function(T, U) { return this.dt = T, this.rows = U, this };
    Q.prototype.build = function(T) {
        var U, V = H("tr"),
            W = this.dt.headings;
        return W.length || (W = T.map(function() { return "" })), F(W, function(X, Y) { U = H("td"), T[Y] || T[Y].length || (T[Y] = ""), U.innerHTML = T[Y], U.data = T[Y], V.appendChild(U) }), V
    }, Q.prototype.render = function(T) { return T }, Q.prototype.add = function(T) {
        if (C(T)) {
            var U = this.dt;
            C(T[0]) ? F(T, function(V) { U.data.push(this.build(V)) }, this) : U.data.push(this.build(T)), U.data.length && (U.hasRows = !0), this.update(), U.columns().rebuild()
        }
    }, Q.prototype.remove = function(T) {
        var U = this.dt;
        C(T) ? (T.sort(function(V, W) { return W - V }), F(T, function(V) { U.data.splice(V, 1) })) : U.data.splice(T, 1), this.update(), U.columns().rebuild()
    }, Q.prototype.update = function() { F(this.dt.data, function(T, U) { T.dataIndex = U }) };
    var R = function(T, U) {
    ...
    return S.init = function(T) {
        ...
        if (U.body = U.table.tBodies[0], U.head = U.table.tHead, U.foot = U.table.tFoot, U.body || (U.body = H("tbody"), U.table.appendChild(U.body)), U.hasRows = 0 < U.body.rows.length, !U.head) {
            var ba = H("thead"),
                ca = H("tr");
            U.hasRows && (F(U.body.rows[0].cells, function() { ca.appendChild(H("th")) }), ba.appendChild(ca)), U.head = ba, U.table.insertBefore(U.head, U.body), U.hiddenHeader = !V.ajax
        }
        ...
        if (this.hasRows && this.totalPages) {
            ...
            this.hasHeadings && (I(this.header, this.isIE), F(this.activeHeadings, function($) { this.header.appendChild($) }, this)), F(this.pages[T], function($) { U.appendChild(this.rows().render($)) }, this), this.clear(U), this.onFirstPage = 1 === this.currentPage, this.onLastPage = this.currentPage === this.lastPage
        } else this.clear();
        var Y, V = 0,
            W = 0,
            X = 0;
        ...
        if (I(this.pagers, this.isIE), 1 < this.totalPages) {
            ...
            var X = this.links;
            this.options.truncatePager && (X = M(this.links, this.currentPage, this.pages.length, this.options.pagerDelta, this.options.ellipsisText)), K.add(this.links[this.currentPage - 1], "active"), F(X, function(Y) { K.remove(Y, "active"), U.appendChild(Y) }), K.add(this.links[this.currentPage - 1], "active"), this.options.nextPrev && U.appendChild(J(T, W, this.options.nextText)), this.options.firstLast && U.appendChild(J(T, this.totalPages, this.options.lastText)), F(this.pagers, function(Y) { Y.appendChild(U.cloneNode(!0)) })
        }
        ...
        U.searchable && (T.input = T.wrapper.querySelector(".dataTable-input"), T.input && G(T.input, "keyup", function() { T.search(this.value) })), G(T.wrapper, "click", function(W) { var X = W.target; "a" === X.nodeName.toLowerCase() && (X.hasAttribute("data-page") ? (T.page(X.getAttribute("data-page")), W.preventDefault()) : U.sortable && K.contains(X, "dataTable-sorter") && "false" != X.parentNode.getAttribute("data-sortable") && (T.columns().sort(T.activeHeadings.indexOf(X.parentNode) + 1), W.preventDefault())) })
    }, S.setColumns = function(T) {
        var U = this;
        T || F(U.data, function(V) { F(V.cells, function(W) { W.data = W.innerHTML }) }), U.options.columns && U.headings.length && F(U.options.columns, function(V) {
            C(V.select) || (V.select = [V.select]), V.hasOwnProperty("render") && "function" == typeof V.render && (U.selectedColumns = U.selectedColumns.concat(V.select), U.columnRenderers.push({ columns: V.select, renderer: V.render })), F(V.select, function(W) {
                var X = U.headings[W];
        ...
        }), U.hasRows && (F(U.data, function(V, W) { V.dataIndex = W, F(V.cells, function(X) { X.data = X.innerHTML }) }), U.selectedColumns.length && F(U.data, function(V) { F(V.cells, function(W, X) {-1 < U.selectedColumns.indexOf(X) && F(U.columnRenderers, function(Y) {-1 < Y.columns.indexOf(X) && (W.innerHTML = Y.renderer.call(U, W.data, W, V)) }) }) }), U.columns().rebuild()), U.render("header")
    }, S.destroy = function() { this.table.innerHTML = this.initialLayout, K.remove(this.table, "dataTable-table"), this.wrapper.parentNode.replaceChild(this.table, this.wrapper), this.initialized = !1 }, S.update = function() {
        ...
        var T = this.options.perPage,
            U = this.activeRows;
        return this.searching && (U = [], F(this.searchData, function(V) { U.push(this.activeRows[V]) }, this)), this.pages = U.map(function(V, W) { return 0 == W % T ? U.slice(W, W + T) : null }).filter(function(V) { return V }), this.totalPages = this.lastPage = this.pages.length, this.totalPages
    }, S.fixColumns = function() {
        if (this.options.fixedColumns && this.activeHeadings && this.activeHeadings.length) {
            var T, U = !1;
            if (this.columnWidths = [], this.table.tHead) F(this.activeHeadings, function(Y) { Y.style.width = "" }, this), F(this.activeHeadings, function(Y, Z) {
                var $ = Y.offsetWidth,
                ...
                }, this), F(this.data, function(Y) { F(Y.cells, function(Z, $) { this.columns(Z.cellIndex).visible() && (Z.style.width = X[$] + "%") }, this) }, this), this.table.removeChild(U)
            }
        }
    }, S.fixHeight = function() { this.options.fixedHeight && (this.container.style.height = null, this.rect = this.container.getBoundingClientRect(), this.container.style.height = this.rect.height + "px") }, S.search = function(T) {
    ...
    }, S.page = function(T) { return T != this.currentPage && (isNaN(T) || (this.currentPage = parseInt(T, 10)), T > this.pages.length || 0 > T ? !1 : void(this.render("page"), this.render("pager"), this.emit("datatable.page", T))) }, S.sortColumn = function(T, U) { this.columns().sort(T, U) }, S.insert = function(T) {
        var U = this,
            V = [];
        if (B(T)) {
    ...
    }, S.refresh = function() { this.options.searchable && (this.input.value = "", this.searching = !1), this.currentPage = 1, this.onFirstPage = !0, this.update(), this.emit("datatable.refresh") }, S.clear = function(T) {
        ...
        if (T = E({ lineDelimiter: "\n", columnDelimiter: "," }, T), T.data.length || B(T.data)) {
            if ("csv" === T.type) {
                ...
                W.length && (T.headings && (U.headings = W[0].split(T.columnDelimiter), W.shift()), F(W, function(Y, Z) {
                    ...
                    $.length && F($, function(_) { U.data[Z].push(_) })
                }))
            } else if ("json" === T.type) {
                var X = D(T.data);
        ...
        var T = this.activeHeadings,
            ...
            X = H("tbody"),
            Y = H("tr");
        F(T, function($) { Y.appendChild(H("th", { html: $.textContent })) }), W.appendChild(Y), F(U, function($) {
            var _ = H("tr");
            F($.cells, function(aa) { _.appendChild(H("td", { html: aa.textContent })) }), X.appendChild(_)
    ...
    }, S.columns = function(T) { return new P(this, T) }, S.rows = function(T) { return new Q(this, T) }, S.on = function(T, U) { this.events = this.events || {}, this.events[T] = this.events[T] || [], this.events[T].push(U) }, S.off = function(T, U) { this.events = this.events || {}, !1 == T in this.events || this.events[T].splice(this.events[T].indexOf(U), 1) }, S.emit = function(T) {
